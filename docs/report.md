...
1. 可扩展性优良
    1. 关于可扩展性的相关说明，可见上面对回调函数注册机制的详细说明。
2. 用户交互友好
    1. 结合使用键盘与鼠标，增加了用户交互操作的丰富性，可以见上面的测试结果说明。


## 项目难点


1. OpenGL很难，接触到底层的知识，底层接口不好用
    1. OpenGL接口规范在计算机图形学中属于一个较为底层的接口，在此接口上，我们需要深刻理解OpenGL的工作原理，并使用OpenGL实现这样的一个简洁的3D建模工具并不容易。
2. 线性代数的需求
